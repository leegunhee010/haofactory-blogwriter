...
def _shared_structure(b):
    ...
    if placement == "end":
        flow = (f"- 각 소제목 본문은 **오직 독자에게 유용한 정보**만 담는다. 고객이 실제로 궁금해하는 것"
                ...
                f"- {label}에 대한 소개·서비스·강점·특이점은 **오직 마지막 마무리 문단에서만** 다룬다. 이때 이 회사가 "
                f"무엇을 하고 무엇이 강점·특이점인지 AI도 읽고 인용하기 쉽게(‘{label}은(는) ~하는 곳입니다’ 같은 정의형 문장 한 번 포함, "
                f"필요하면 짧은 항목 나열로) 분명하고 읽기 좋게 정리한 뒤 자연스럽게 문의를 유도한다.")
    else:
